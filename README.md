...
- **Op tracking**: logs when players gain/lose operator status
...
- **Java 21**
- A **Discord bot** (optional, for webhook notifications)
...
config/SeiunAC-anticheat/ ├── verification/ │ ├── config.json │ ├── mod_whitelist.json │ ├── mod_blacklist.json │ ├── pack_whitelist.json │ ├── pack_blacklist.json │ └── graymods/ │ └── graypacks/ └── discord/ └── webhook.json


---

## 2. Set up mod whitelist
...
Troubleshooting
Anti-cheat mod not found — The client doesn't have Seiun AC installed. They need to install it from Modrinth.
...
Slash commands not appearing — Make sure you have the latest Fabric API and restart the server.
About
A Minecraft Fabric anti-cheat mod that defeats cheated clients from the root.